...
import {scandirExt, type ScandirEntry} from "@aniojs/node-fs"
...
const executeStep: Preprocess = async function(session) {
	const projectFilesAndDirectoriesOnDiskByRelativePath: Map<string, ScandirEntry> = new Map()

	const {entries} = await scandirExt(
		path.join(session.projectRoot, "project")
	)

	for (const entry of entries) {
		projectFilesAndDirectoriesOnDiskByRelativePath.set(
	...
	await _replicateDirectoryTree(
		session.projectRoot,
		projectFilesAndDirectoriesOnDiskByRelativePath
	)
